...
import { setupImRoutes } from './routes/im';
...
const initStore = async (): Promise<SqliteStore> => {
  if (!store) {
    store = await SqliteStore.create(getUserDataPath());
  }
  return store;
};
...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      console.log(`[Server] ${APP_NAME} Web Server running on port ${PORT}`);
      console.log(`[Server] WebSocket server initialized`);
      console.log(`[Server] User data path: ${getUserDataPath()}`);
